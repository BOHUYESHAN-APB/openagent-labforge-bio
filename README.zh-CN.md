...
> **我们只推荐使用最新版本。** 旧版本不再维护，可能存在 bug、安全问题或配置不兼容。

每次大版本号变更后（如 1.x → 2.x），我们会删除前一个大版本系列的所有 release。例如，当 v2.0.0 发布后，所有 v1.x.x 的 release 将被移除。这确保用户始终运行受支持、经过测试的版本。如需特定旧版本，请在清理前 fork 仓库并检出对应 tag。
...
| `chem-orchestrator` | chem-analyst | **化学专家** — 计算化学、分子建模 |
...
> **v1.3.5+**：旧的 `subagentPolicy` 系统（ultra-minimal/minimal/full/custom/main-only 模式和 `/ol-subagents-*` 指令）已移除。Agent 注册仅使用 `disabled_agents` 配置。
