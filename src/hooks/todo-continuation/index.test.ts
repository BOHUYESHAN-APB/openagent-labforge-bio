...
describe('createTodoContinuationHook', () => {
  ...
  describe('todo hygiene routing', () => {
    ...
    test('exposes a system transform handler', async () => {
      const ctx = createMockContext({
        todoResult: {
          data: [
            {
      ...
      expect('handleSystemTransform' in hook).toBe(true);
    });

    test('injects hygiene reminder into system after todowrite activity', async () => {
      const ctx = createMockContext({
        todoResult: {
          data: [
            { id: '1', content: 'todo1', status: 'pending', priority: 'high' },
    ...
    test('compaction-like transform does not consume pending reminder', async () => {
      ...
      const systemClone = { system: ['base'] };
      await hook.handleSystemTransform({ sessionID: 'main1' }, systemClone);
      expect(systemClone.system.join('\n')).toContain(TODO_HYGIENE_REMINDER);

      await hook.handleMessagesTransform(live);
      const systemLive = { system: ['base'] };
      await hook.handleSystemTransform({ sessionID: 'main1' }, systemLive);
      expect(systemLive.system.join('\n')).toContain(TODO_HYGIENE_REMINDER);
    });
    ...
    test('new request clears stale pending reminder state', async () => {
      ...
      const systemBlocked = { system: ['base'] };
      await hook.handleSystemTransform({ sessionID: 'main1' }, systemBlocked);
      expect(systemBlocked.system.join('\n')).not.toContain(
        TODO_HYGIENE_REMINDER,
      );
      ...
      const systemAllowed = { system: ['base'] };
      await hook.handleSystemTransform({ sessionID: 'main1' }, systemAllowed);
