...
*** End Patch`,
    ...
    // Windows doesn't support Unix file permissions via chmod
    if (!isWindows) {
...
*** End Patch`,
  ...
  test('applyPreparedChanges reverts previous changes when a later apply fails', async () => {
    ...
    // Windows doesn't support Unix file permissions via chmod
    if (!isWindows) {
      expect((await stat(path.join(root, 'first.txt'))).mode & 0o777).toBe(
        0o755,
      );
    }
...
*** End Patch`,
    ...
    if (!isWindows) {
      expect(
        (await stat(path.join(root, 'nested/after.txt'))).mode & 0o777,
      ).toBe(0o755);
    }
