...
describe('loadPluginConfig', () => {
  ...
  test('prefers new extendai-lab project config basename', () => {
    const projectDir = path.join(tempDir, 'project');
    const projectConfigDir = path.join(projectDir, '.opencode');
    fs.mkdirSync(projectConfigDir, { recursive: true });
    fs.writeFileSync(
      path.join(projectConfigDir, `${CONFIG_BASENAME}.json`),
      JSON.stringify({
        agents: {
          oracle: { model: 'extend/model' },
        },
      }),
    );

    const config = loadPluginConfig(projectDir);
    expect(config.agents?.oracle?.model).toBe('extend/model');
  });
