...
/**
 * Load a model preset from the presets directory.
 * Searches in system config directories for presets/{name}.json
 *
 * @param presetName - Name of the preset (openai, deepseek, mixed)
 * @returns Preset config object or null if not found
 */
export function loadModelPreset(
  presetName: string,
): { modelPreferences?: PluginConfig['modelPreferences'] } | null {
  const searchDirs = getConfigSearchDirs();
  for (const configDir of searchDirs) {
...
export function loadPluginConfig(directory: string): PluginConfig {
  ...
  // Load model preset if modelPreferences.profile is set
  if (config.modelPreferences?.profile) {
    const modelPreset = loadModelPreset(config.modelPreferences.profile);
    if (modelPreset?.modelPreferences) {
      // Merge preset model preferences with user config (user config takes precedence)
      config.modelPreferences = {
        ...modelPreset.modelPreferences,
        ...config.modelPreferences,
        perAgent: {
          ...modelPreset.modelPreferences.perAgent,
          ...config.modelPreferences.perAgent,
        },
      };
    }
  }
