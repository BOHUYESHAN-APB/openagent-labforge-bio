...
function getPreviousAttempt(
  ...
  const attemptIndex = task.attempts.findIndex(
...
export class BackgroundManager {
    ...
    this.parentWakeNotifier = new ParentWakeNotifier(
      {
        ...
        enqueueNotificationForParent:
          this.enqueueNotificationForParent.bind(this),
      },
      {
  ...
  private addTask(task: BackgroundTask): void {
    ...
    const taskIDs =
      this.tasksByParentSession.get(task.parentSessionId) ?? new Set<string>();
    taskIDs.add(task.id);
  ...
  private updateTaskParent(
    ...
    const taskIDs =
      this.tasksByParentSession.get(parentSessionID) ?? new Set<string>();
    taskIDs.add(task.id);
  ...
  private async startTask(item: QueueItem): Promise<void> {
    ...
    if (task.retryNotification) {
      ...
      const failedSessionID =
        previousAttempt?.sessionId ?? task.retryNotification.previousSessionID;
      const failedSessionLine = failedSessionID
        ? `\n- Failed session: \`${failedSessionID}\``
        : '';
      const failedModel =
        formatAttemptModelSummary(previousAttempt) ??
        task.retryNotification.failedModel;
      const failedModelLine = failedModel
        ? `\n- Failed model: \`${failedModel}\``
        : '';
      const failedError =
        previousAttempt?.error ?? task.retryNotification.failedError;
      const failedErrorLine = failedError ? `\n- Error: ${failedError}` : '';
      const retryModel =
        formatAttemptModelSummary(boundAttempt) ??
        task.retryNotification.nextModel;
      const parentPromptContext =
...
The fallback retry session is now created and can be inspected directly.
  ...
  getTask(id: string): BackgroundTask | undefined {
    return (
      this.tasks.get(id) ??
      this.completedTaskArchive.get(id) ??
          ...
          if (isAmbiguousPostDispatchPromptFailure(promptResult)) {
            log(
              '[background-agent] resume prompt may have been accepted before ambiguous failure; continuing to poll',
              {
                ...
                sessionID: existingTask.sessionId,
                error:
                  promptResult.error instanceof Error
  ...
  handleEvent(event: Event): void {
    ...
    if (event.type === 'session.idle') {
      ...
      const sessionID = resolveSessionEventID(props);
      if (sessionID) {
        void this.enqueueNotificationForParent(sessionID, () =>
    ...
    if (event.type === 'session.error') {
      ...
      if (!resolved?.isCurrent) {
        void this.requeueDispatchedParentWake(sessionID, 'session.error').catch(
          (error) => {
            log(
  ...
  private async handleSessionErrorEvent(args: {
    ...
    if (!task.fallbackChain && task.sessionId) {
      const sessionFallbackChain =
        this.modelFallbackControllerAccessor?.getSessionFallbackChain(
          task.sessionId,
        );
...
The task was re-queued on a fallback model after a retryable failure.
  ...
  queuePendingNotification(
    ...
    const existingNotifications =
      this.pendingNotifications.get(sessionID) ?? [];
  ...
  private async notifyParentSession(task: BackgroundTask): Promise<void> {
    ...
    if (this.enableParentSessionNotifications) {
      ...
      if (shouldDeferNotification) {
        ...
        log(
          '[background-agent] Deferred notification until parent session is idle:',
          {
            taskId: task.id,
            allComplete,
            isTaskFailure,
    ...
    try {
      ...
      for (const task of this.tasks.values()) {
        ...
        try {
          ...
          const sessionGoneFromStatus =
            allStatuses !== undefined && !sessionStatus;
