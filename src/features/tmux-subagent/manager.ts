...
export class TmuxSessionManager {
  ...
  constructor(
    ...
    try {
      ...
      this.deps.log(
        '[tmux-session-manager] failed to parse server URL, using fallback',
        {
  ...
  private async spawnInIsolatedContainer(
    ...
    if (this.isolatedWindowPaneId) {
      const state = await this.deps
        .queryWindowState(this.isolatedWindowPaneId)
        .catch((error) => {
          this.deps.log(
  ...
  private async finalizeForceRemoveCandidate(
    ...
    if (this.windowStateContainsPane(state, tracked.paneId)) {
      this.deps.log(
        '[tmux-session-manager] pane still exists after max close retries; manual intervention required',
        {
          sessionId: tracked.sessionId,
          paneId: tracked.paneId,
          source,
  ...
  private sweepExpiredFailedReadinessSessions(): void {
    ...
    for (const [
      sessionId,
      failedReadinessSession,
    ] of this.failedReadinessSessions.entries()) {
  ...
  private getFailedReadinessSession(
    sessionId: string,
  ): FailedReadinessSession | undefined {
    ...
    if (
      !this.isFailedReadinessSessionExpired(failedReadinessSession, Date.now())
    ) {
  ...
  private async tryAttachDeferredSession(): Promise<void> {
    ...
    try {
      ...
      if (deferred.retryIsolatedContainer) {
        const readyForIsolatedContainer =
          await this.ensureSessionReadyBeforeSpawn(
            sessionId,
            'deferred.isolated-container',
          );
  ...
  private async sweepStaleIsolatedSessionsOnce(): Promise<void> {
    ...
    try {
      ...
      if (killed > 0) {
        this.deps.log('[tmux-session-manager] stale isolated sessions swept', {
          killed,
        });
      }
      this.staleSweepCompleted = true;
