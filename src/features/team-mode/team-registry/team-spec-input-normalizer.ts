...
function deriveMemberNameStem(member: JsonRecord): string {
  ...
  if (
    ...
    typeof member.subagent_type === 'string'
  ) {
...
function hasMemberLeadFlag(rawMembers: unknown[]): boolean {
  return rawMembers.some(
    (member) => isJsonRecord(member) && member.isLead === true,
  );
}
...
export function normalizeTeamSpecInput(
  ...
  let leadAgentId =
    typeof raw.leadAgentId === 'string' ? raw.leadAgentId : undefined;
  ...
  if (Array.isArray(rawMembers)) {
    ...
    const shouldUseFirstMemberAsLead =
      !hasExplicitLead &&
    ...
    if (
      ...
      if (
        normalizedMembers.some(
          (member) => getMemberName(member) === normalizedLeadAgentId,
        )
      ) {
