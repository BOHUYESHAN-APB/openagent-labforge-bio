...
function translateMemberError(
  ...
  if (input.kind === 'category' || (!hasKind && hasCategory)) {
    const category =
      typeof input.category === 'string' ? input.category : '<unknown>';
    return new MemberValidationError(
  ...
  if (input.kind === 'subagent_type' || (!hasKind && hasSubagentType)) {
    ...
    if (
      ...
      return new MemberValidationError(
        `Unknown subagent_type '${subagentType}'. Available ELIGIBLE agents: sisyphus, atlas, sisyphus-junior, hephaestus (if D-36 applied). Use an orchestrator-managed delegation lane for read-only specialists like oracle, librarian, explore, metis, momus, multimodal-looker.`,
        name,
        'unknown-subagent',
      );
    }
