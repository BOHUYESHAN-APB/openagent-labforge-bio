...
function createRuntimeState(
  ...
  return {
    ...
    members: spec.members.map((member) => ({
      name: member.name,
      sessionId:
        member.name === spec.leadAgentId ? undefined : `${member.name}-session`,
...
export const deleteTeamMock = mock(
  async (
    teamRunId: string,
    _config?: unknown,
    _tmuxMgr?: unknown,
...
export const approveShutdownMock = mock(
  async (teamRunId: string, memberName: string) => {
    ...
    const member = runtimeState.members.find(
...
export const rejectShutdownMock = mock(
  async (teamRunId: string, memberName: string, reason: string) => {
