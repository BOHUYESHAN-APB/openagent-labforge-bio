...
import { listActiveTeams, loadRuntimeState } from '../team-state-store/index';
import {
  createTask,
  getTask,
  listTasks,
  ...
  inline_spec: {
    ...
    args: {
      teamName: z
        ...
        .min(1)
        .optional()
        .describe('Name of an existing team to activate'),
      inline_spec: z
        .unknown()
        .optional()
        .describe('Inline TeamSpec to create a new team'),
    },
    async execute(args, toolContext) {
      try {
        ...
        if (typeof raw === 'string') {
          try {
            inlineSpec = JSON.parse(raw);
          } catch {
            return 'Error: inline_spec is a string but not valid JSON.';
          }
...
Use to: "lead" for the lead, "<name>" for a specific teammate, "*" for team-wide broadcasts.`,
    ...
    async execute(args) {
      try {
        ...
        const senderMember = runtimeState.members.find(
          (m) => m.agentType === 'leader',
        );
  ...
  return tool({
    ...
    args: {
      ...
      targetMemberName: z
        .string()
        .min(1)
        .describe('Name of the member to shut down'),
    },
    async execute(args, toolContext) {
      try {
  ...
  return tool({
    ...
    args: {
      ...
      memberName: z
        .string()
        .min(1)
        .describe('Name of the member whose shutdown to approve'),
    },
    async execute(args, toolContext) {
      try {
        const state = await loadRuntimeState(args.teamName);
  ...
  return tool({
    ...
    args: {
      ...
      memberName: z
        .string()
        .min(1)
        .describe('Name of the member whose shutdown to reject'),
