...
type TmuxCommandResult = Awaited<
  ReturnType<typeof sharedTmuxModule.runTmuxCommand>
>;
...
export type TeamLayoutDeps = {
  runTmuxCommand: (
    tmuxPath: string,
    args: Array<string>,
    options?: Parameters<typeof sharedTmuxModule.runTmuxCommand>[2],
...
function selectExistingTeammatePane(
  ...
  return (
    teammatePanes[Math.floor(teammatePanes.length / 2)] ??
...
function isTeamLayoutDeps(
  ...
  return (
    value !== undefined && 'runTmuxCommand' in value && 'getTmuxPath' in value
  );
}
...
function isTeamLayoutCleanupTarget(
  ...
  return (
    value !== undefined && 'ownedSession' in value && 'targetSessionId' in value
  );
}
