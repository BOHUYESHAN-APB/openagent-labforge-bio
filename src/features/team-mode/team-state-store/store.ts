  ...
  const runtimeState = validateRuntimeState(
    {
      ...
      members: spec.members.map((member) => ({
        name: member.name,
        agentType:
          spec.leadAgentId === member.name ? 'leader' : 'general-purpose',
        status: 'pending',
        color: member.color,
...
export async function transitionRuntimeState(
  ...
  return withLock(
    path.join(runtimeDirectoryPath, 'state.lock'),
    async () => {
...
export async function listActiveTeams(config: TeamModeConfig): Promise<
  Array<{
    teamRunId: string;
    teamName: string;
    status: string;
