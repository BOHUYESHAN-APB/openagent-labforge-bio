...
async function cleanupMemberWorktrees(
...
function isCreatingStateStuck(
  runtimeState: RuntimeState,
  now: number,
): boolean {
  return (
    runtimeState.status === 'creating' &&
    now - runtimeState.createdAt > CREATING_TIMEOUT_MS
  );
}
        ...
        currentMember.name === member.name
          ? {
              ...
              pendingInjectedMessageIds:
                currentMember.pendingInjectedMessageIds.filter(
                  (messageId) => !reclaimedMessageIdSet.has(messageId),
                ),
            }
          : currentMember,
      ),
...
async function inspectWorkerMembers(
  ...
  const workerMembers = runtimeState.members.filter(
    (member) => member.agentType !== 'leader',
  );

  return await Promise.all(
    workerMembers.map(async (member) => {
    ...
    try {
      ...
      switch (runtimeState.status) {
        ...
        case 'active': {
          ...
          const workerCheckResults = await inspectWorkerMembers(
            ctx,
            runtimeState,
          );
          ...
          const hasAliveWorker = workerCheckResults.some(
            (result) => result.stillAlive,
          );
          const hasAnyWorker = workerCheckResults.length > 0;

          const markDeadWorkersErrored = (
