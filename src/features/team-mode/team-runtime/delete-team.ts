...
export async function deleteTeam(
  ...
  const nonLeadMembers = runtimeState.members.filter(
    (member) => member.agentType !== 'leader',
  );

  if (bgMgr && runtimeState.leadSessionId) {
    const teamMessageMarkerPrefix = `team-create:${teamRunId}:`;
    const teamTasks = bgMgr
      .getTasksByParentSession(runtimeState.leadSessionId)
      .filter(
        (task) =>
          task.teamRunId === teamRunId ||
  ...
  if (options?.force === true) {
    ...
    nonLeadMembers.some(
      (member) => !DELETABLE_MEMBER_STATUSES.has(member.status),
    )
  ) {
    throw new Error('members still active');
  }

  const deletableTeamStatuses =
    options?.force === true
  ...
  const removedLayout =
    config.tmux_visualization && tmuxMgr !== undefined && deps.canVisualize();
  if (removedLayout) {
    const memberPaneIds = runtimeState.members.flatMap((member) =>
  ...
  sweepStaleTeamSessions(
    new Set(activeTeams.map((team) => team.teamRunId)),
  ).catch(() => {});
