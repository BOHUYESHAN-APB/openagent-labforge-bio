...
function detectRawPromptInSnippet(contents: string): boolean {
  ...
  const visit = (node: ts.Node): void => {
    ...
    const isRawPromptAccess =
      isRawPromptPropertyAccess(node) && !isTypeofPromptCheck(node);
    if (
      isRawPromptAccess ||
      isPromptBindingPattern(node) ||
...
function findPromptGateCallsWithoutQueueBehavior(
  filePath: string,
  contents: string,
): number[] {
  const sourceFile = ts.createSourceFile(
...
function findPromptRetryCallsWithoutQueueBehavior(
  filePath: string,
  contents: string,
): number[] {
  const sourceFile = ts.createSourceFile(
...
`;
    ...
    // when
    const offenders = findPromptGateCallsWithoutQueueBehavior(
      'audit-snippet.ts',
      snippet,
    );
  ...
  test('#given production TypeScript sources #when prompt gate callers are audited #then every route declares queue behavior explicitly', async () => {
    ...
    for (const filePath of files) {
      ...
      const missingLines = findPromptGateCallsWithoutQueueBehavior(
        filePath,
        contents,
      );
  ...
  test('#given production TypeScript sources #when model-suggestion prompt wrappers are audited #then every retry caller declares queue behavior explicitly', async () => {
    ...
    for (const filePath of files) {
      ...
      const missingLines = findPromptRetryCallsWithoutQueueBehavior(
        filePath,
        contents,
      );
      for (const line of missingLines) {
