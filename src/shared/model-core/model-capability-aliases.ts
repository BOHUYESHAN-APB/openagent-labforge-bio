...
const EXACT_ALIAS_RULES: ReadonlyArray<ExactAliasRule> = [
  ...
  },
  {
    ...
    rationale:
      'Kimi for Coding exposes k2pb while the bundled capabilities snapshot uses the canonical k2p5 ID.',
  },
  {
    ...
    rationale:
      'GitHub Copilot exposes Claude Opus 4.7 with dotted version syntax while the snapshot uses dashed syntax.',
  },
];
...
export function resolveModelIDAlias(modelID: string): ModelIDAliasResolution {
  ...
  const aliasLookupModelID =
    stripProviderPrefixForAliasLookup(requestedModelID);
