...
export async function dispatchInternalPrompt<TInput = PromptAsyncInput>(
  ...
  const postDispatchHoldMs =
    args.postDispatchHoldMs ?? DEFAULT_PROMPT_ASYNC_POST_DISPATCH_HOLD_MS;
  const dispatchTimeoutMs =
    args.dispatchTimeoutMs ?? DEFAULT_PROMPT_DISPATCH_TIMEOUT_MS;
  ...
  const queueBehavior =
    args.queueBehavior ?? (args.mode === 'sync' ? 'defer' : 'enqueue');
