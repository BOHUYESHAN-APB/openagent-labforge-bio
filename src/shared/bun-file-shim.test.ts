...
describe('bun-file-shim', () => {
  ...
  describe('#given Node fallback without Bun global', () => {
    beforeAll(async () => {
      nodeFallbackTemporaryDirectory = await mkdtemp(
        join(tmpdir(), 'bun-file-shim-node-'),
      );
    });
    ...
    afterAll(async () => {
      ...
      await rm(nodeFallbackTemporaryDirectory, {
        recursive: true,
        force: true,
      });
    });
