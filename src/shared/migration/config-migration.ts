...
export function migrateConfigFile(
  ...
  const shouldWriteSidecar =
    newMigrationsToRecord.length > 0 ||
    hadLegacyInConfigMigrations ||
    hadInlineAppliedMigrations;
  ...
  if (copy.lsp !== undefined) {
    const droppedServers =
      copy.lsp && typeof copy.lsp === 'object'
