...
function loadNodeFallbackBunWhich(): BunWhichFunction {
  ...
  if (
    !source.includes(fsImport) ||
    !source.includes(pathImport) ||
    !source.includes(exportSignature)
  ) {
  ...
  const scriptSource = `${source
    ...
    .replace(
      exportSignature,
      'function bunWhich(commandName: string): string | null {',
