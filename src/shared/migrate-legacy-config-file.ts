...
function archiveLegacyConfigFile(legacyPath: string): boolean {
  ...
  try {
    ...
    log(
      '[migrateLegacyConfigFile] Legacy config was migrated and renamed to backup. Update the canonical file only.',
      {
        legacyPath,
        backupPath,
      },
    ...
    try {
      rmSync(legacyPath);
      log(
        '[migrateLegacyConfigFile] Legacy config was migrated and removed after backup rename failed. Update the canonical file only.',
        {
          legacyPath,
          backupPath,
          renameError,
      ...
      log(
        '[migrateLegacyConfigFile] WARNING: canonical config was written but the legacy file still exists and will be ignored. Remove or rename it manually.',
        {
          legacyPath,
          backupPath,
          renameError,
...
function migrateLegacySidecarFile(
  legacyPath: string,
  ...
  try {
    ...
    log(
      '[migrateLegacyConfigFile] Migrated legacy migration sidecar to canonical path',
      {
    ...
    log(
      '[migrateLegacyConfigFile] Failed to migrate legacy migration sidecar',
      {
        legacySidecarPath,
...
export function migrateLegacyConfigFile(legacyPath: string): boolean {
  ...
  try {
    ...
    log('[migrateLegacyConfigFile] Failed to migrate legacy config file', {
      legacyPath,
      error,
    });
    return false;
