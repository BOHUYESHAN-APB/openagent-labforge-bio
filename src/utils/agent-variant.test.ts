...
describe('resolveRuntimeAgentName', () => {
  ...
  test('resolves default friendly display names to internal names', () => {
    expect(resolveRuntimeAgentName(undefined, 'engineer')).toBe('orchestrator');
    expect(resolveRuntimeAgentName(undefined, 'planner')).toBe('prometheus');
    expect(resolveRuntimeAgentName(undefined, '@executor')).toBe('atlas');
    ...
    expect(resolveRuntimeAgentName(undefined, 'plan-reviewer')).toBe('momus');
  });
...
describe('rewriteDisplayNameMentions', () => {
  ...
  test('rewrites default friendly display-name mentions', () => {
    expect(
      rewriteDisplayNameMentions(
        undefined,
        'ask @engineer to coordinate, @planner to plan, and @executor to execute',
      ),
    ).toBe(
      'ask @orchestrator to coordinate, @prometheus to plan, and @atlas to execute',
    );
    expect(
      rewriteDisplayNameMentions(
        undefined,
  ...
  test('resolves custom agents by displayName for variant/runtime lookups', () => {
    ...
    expect(resolveRuntimeAgentName(config, '@auditor')).toBe('custom-auditor');
    expect(rewriteDisplayNameMentions(config, 'ask @auditor for details')).toBe(
      'ask @custom-auditor for details',
    );
    expect(resolveAgentVariant(config, '@auditor')).toBe('high');
  });
});
