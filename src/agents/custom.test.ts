...
describe("custom-agent creation", () => {
  test("infers custom agents from unknown keys", () => {
    const config: PluginConfig = {
      agents: {
        ...
        codechecker: {
          ...
          prompt: "You are the custom codechecker agent.",
        },
      },
    };
    ...
    expect(names).toContain("codechecker");

    const customAgent = agents.find((agent) => agent.name === "codechecker");
    expect(customAgent).toBeDefined();
    expect(customAgent?.config.model).toBe("openai/gpt-5.5");
    expect(customAgent?.config.prompt).toBe(
      "You are the custom codechecker agent."
    );
  });
  ...
  test("skips custom agents without a model", () => {
    ...
    try {
      ...
      expect(warnSpy).toHaveBeenCalledWith(
        "[openagent-labforge] Custom agent 'janitor' skipped: 'model' is required"
      );
    } finally {
      warnSpy.mockRestore();
    }
